...
public class WindowController {
    ...
    public ResponseEntity<Window> create(@RequestBody WindowCommand window) {
        RoomEntity room = roomDao.findById(window.getRoomId()).orElse(null);

        if (room==null) {
            return ResponseEntity.badRequest().build();
        }

        SensorEntity windowStatus = new SensorEntity(SensorType.STATUS, window.getName() + " status");
        windowStatus.setValue(window.getWindowStatus().getValue());
        sensorDao.save(windowStatus);

        WindowEntity entity = new WindowEntity(window.getName(), windowStatus, room);
